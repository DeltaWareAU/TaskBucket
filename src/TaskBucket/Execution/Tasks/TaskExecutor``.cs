...
{
    internal abstract class TaskExecutor<TExecutor, TResult>(ITaskOptions options) : ITaskExecutor<TResult> where TExecutor : class
    {
        ...
        protected async Task InternalRunTaskAsync(Func<TExecutor, CancellationToken, Task<TResult>> taskExecutor, object executorInstance, int bucketIndex, CancellationToken cancellationToken)
        {
            ...
            try
            {
                Result = await taskExecutor.Invoke(instance, cancellationToken);
